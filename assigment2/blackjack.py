...
deck = list(range(0, 416))  # generate 8 decks of cards
random.shuffle(deck)
...
def draw_card(mydeck):
    """Draws a card and removes it from the deck"""
    card = random.choice(mydeck)
    deck.remove(card)
    return card
...
def player(player):
    ...
    while True:
        ...
        #         if split.lower() in ["y", "n", "yes", "no"] and len(split) < 4:
        #             break
        #         else:
        #              print("Please enter yes/y or no/n !")
        #     if split.lower() in ["y", "yes"]:
        #         for c in cards:
        #             print(f"Running split {c+1} ...\n")
